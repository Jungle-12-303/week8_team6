...
이번 저희 팀의 목표는 Thread Pool과 뮤텍스를 설계와 구현하는것 이었습니다.
그렇기에 저희는 지난주에 만든 SQL 처리기와 B+ Tree 인덱스를  내부 엔진으로 차용하였고,
  HTTP API 서버와 Thread Pool 그리고 뮤텍스를 설계 및 구현하는데 집중하였습니다
...
그렇기에 worker를 20개로 늘려도 SQL 처리량이 5배 늘지 않습니다 저희팀을 이를 인지하고 개선하기 위해
추후 개선한다면 SELECT 같은 읽기 요청은 동시에 허용하고,
INSERT처럼 파일과 인덱스를 변경하는 요청만 막는 구조로 바꿔 병렬성을 높일 수 있다 라고 예상하고있습니다
...
그래서 이후 새 연결들이 아직 accept되지 못해 OS backlog에  4096개까지 남습니다.
이후 들어오는 새요청은 타임아웃 되게 설계하였습니다.
```

## 2:30 - 3:00 부하 로그와 mutex

```text
이 그래프는 서버와의 연결 상태를 확인하는 헬스 요청 과 실제 쿼리 요청을 100번 하였을때 로그를 나타낸것입니다.
/health요청은 DB를 타지 않아서 100번 요청해도 거의 0초 근처에 붙어 있습니다.

반면 /query 요청은 worker가 병렬로 받아도 기존 SQL 엔진 구간은 mutex로 보호되게 설계되었기에.
 뒤쪽 요청으로 갈수록 mutex 앞 대기 시간이 누적되어 total 시간이 증가합니다.
```
...
마지막으로 이번 프로젝트를 통해 무조건적인 뮤택스는 무조건적인 속도 향상을 가져다 주지 않는 다는것을 배우게되었습니다
이상 6팀 발표를 경청해주셔서 감사합니다
```
