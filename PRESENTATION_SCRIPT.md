...
제 PC는 논리 프로세서가 20개지만, DB 처리 구간이 mutex로 보호되어 있어서 SQL 실행은 실제로 병렬 처리되지 않습니다.
따라서 작업 스레드를 20개로 늘려도 처리량이 그만큼 비례해서 늘어나지는 않습니다.

그리고 운영체제의 backlog와 스레드 풀 대기열은 서로 다른 줄입니다.
backlog는 accept되기 전의 커널 대기열이고, 스레드 풀 대기열은 accept된 뒤 작업 스레드를 기다리는 내부 대기열입니다.
내부 대기열이 가득 차면 accept 루프가 잠시 멈추고, 그 사이 새 연결은 운영체제 backlog에 쌓이게 됩니다.
